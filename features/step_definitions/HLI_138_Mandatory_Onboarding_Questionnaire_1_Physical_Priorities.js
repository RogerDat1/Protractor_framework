...
defineSupportCode(function ({Given}) {
    ...
    Given(/^User should see "([^"]*)" as a physical priority at "([^"]*)" screen$/, function (PhysicalPriority, PageName, callback) {
        keyword.expectVisible('Screen_' + PageName + '|physicalPriorityName_' + PhysicalPriority, function () {
            callback();
        })
    });
    Given(/^User clicks on "([^"]*)" physical priority at "([^"]*)" screen$/, function (PhysicalPriority, PageName, callback) {
        keyword.expectVisible('Screen_' + PageName + '|physicalPriorityName_' + PhysicalPriority, function () {
            keyword.performclick('Screen_' + PageName + '|physicalPriorityName_' + PhysicalPriority, function () {
                callback();
            })
        })
    });
    Given(/^User should see physical priority "([^"]*)" header below at "([^"]*)" screen$/, function (PhysicalPriority, PageName, callback) {
        keyword.expectVisible('Screen_' + PageName + '|physicalPriorityHeader_' + PhysicalPriority, function () {
            keyword.getTextOfElement('Screen_' + PageName + '|physicalPriorityHeader_' + PhysicalPriority, function (text) {
                expect(text.toString().trim()).to.equal(HLIData['Screen_' + PageName]['physicalPriorityName_' + PhysicalPriority].header);
                callback();
            })
        })
    });
    Given(/^User should see physical priority "([^"]*)" description below at "([^"]*)" screen$/, function (PhysicalPriority, PageName, callback) {
        keyword.expectVisible('Screen_' + PageName + '|physicalPriorityDescription_' + PhysicalPriority, function () {
            keyword.getTextOfElement('Screen_' + PageName + '|physicalPriorityDescription_' + PhysicalPriority, function (text) {
                expect(text.toString().trim()).to.equal(HLIData['Screen_' + PageName]['physicalPriorityName_' + PhysicalPriority].about);
                callback();
            })
        })
    });

    Given(/^User see existing order of physical priorities at "([^"]*)" screen$/, function (PageName, callback) {
    ...
    Given(/^User drags a physical priority and drop at "([^"]*)" screen$/, function (PageName, callback) {
        keyword.dragNdrop('Screen_PhysicalPriorities|first', 'Screen_PhysicalPriorities|Five', function () {
            callback();
        })
    });

    Given(/^User see new order of physical priorities at "([^"]*)" screen$/, function (PageName, callback) {
    ...
    Given(/^Existing and new order should be different at Physical Priorities screen$/, function (callback) {
        expect(JSON.stringify(orderBeforeDragAndDrop) === JSON.stringify(orderAfterDragAndDrop)).to.equal(false);
        callback();
    });


    Given(/^User should be navigated to Health age page$/, function (callback) {
        keyword.expectVisible('health_Summary|lnk_Data', function () {
            callback();
        })
    })
    Given(/^User see default order of physical priorities at "([^"]*)" screen$/, function (PageName, callback) {
        getAllPhysicalPrioritiesName(function (order1) {
            orderBeforeDragAndDrop = order1;
            console.log(orderBeforeDragAndDrop)
    ...
    Given(/^User clicks on next without modifying default order of physical priorities at "([^"]*)" screen$/, function (PageName, callback) {
